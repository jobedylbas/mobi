monitorDAO = function (connection){
...
monitorDAO.prototype.getPC = function (res) { 
  // Abre a conexão com o DB
    ...
    this._connection.collection("pcs", function (err, collection){
  ...
  this._connection.open( function(err, mongoclient){
    ...
    mongoclient.collection( colName, function (err, collection){ // recuperar a partir do id
      ...
      collection.find().sort({$natural: -1}).limit(1).toArray(function(err, result){
        ...
        res.send(JSON.stringify({ "timestamp": result.timestamp, dataKey: result[dataType][dataKey]}));
      });
    });
  });
}
